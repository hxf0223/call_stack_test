...
template <typename F> class FrameUnwinder {
...
private:
  ...
  _Unwind_Reason_Code backtrace(_Unwind_Context *ctx) {
    if (m_index >= 0 && static_cast<size_t>(m_index) >= m_depth) return _URC_END_OF_STACK;
...
template <typename F> void unwind_nth_frame(F &f, size_t frame_number) {
